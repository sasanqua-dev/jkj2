...
// フォトリフレクタ（3センサ構成：左・中央・右）
const int sensor_l = A2; // 左センサ
const int sensor_c = A3; // 中央センサ
...
void loop() { // 制御プログラム（3センサによるライントレース）
  // 3つのセンサをアナログ値で読み取る
  val_l = analogRead(sensor_l);
  val_c = analogRead(sensor_c);
  ...
  //  ---
  //  010 … 中央のみ黒 → 直進
  //  110 … 左＋中央が黒 → 右にずれているので緩やかに左へ
  //  100 … 左のみ黒    → 大きく左へ旋回
  //  011 … 中央＋右が黒 → 左にずれているので緩やかに右へ
  //  001 … 右のみ黒    → 大きく右へ旋回
  //  111 … 全センサ黒  → 交差点／太線と判断してそのまま直進
  //  000 … 全センサ白  → 線を見失ったので直前の曲がった方向へ復帰
  if (on_c && !on_l && !on_r) {
    // 中央のみ → 直進
    forward(traceSpeed);
  } else if (on_l && on_c && !on_r) {
    // 左＋中央 → 緩やかに左へ
    curveLeft(traceSpeed, 50);
    last_turn_flag = 2;
  } else if (on_l && !on_c && !on_r) {
    // 左のみ → 鋭く左へ
    turnLeft(traceSpeed);
    ...
    // 全センサ黒（交差点・太線）→ とりあえず直進
    forward(traceSpeed);
  } else {
    // 全センサ白 → 線を見失ったので直前の曲がった方向へ復帰
    if (last_turn_flag == 1) {
      turnRight(traceSpeed);
